...
async function CreateIncident(call, callback) {
  try {
    ...
    const stmt = db.prepare(`
      ...
      VALUES (?, ?, ?, ?, 'OPEN', ?, '', '', ?, ?)
    `);
    await stmt.run(id, title, description || '', severity.toUpperCase(), service_name || '', now, now);
...
async function GetIncident(call, callback) {
  try {
    const incident = await db.prepare('SELECT * FROM incidents WHERE id = ?').get(call.request.id);
...
async function ListIncidents(call, callback) {
  try {
    const { status } = call.request;
    let incidents;
    if (!status || status === 'ALL') {
      incidents = await db.prepare('SELECT * FROM incidents ORDER BY created_at DESC').all();
    } else {
      incidents = await db.prepare('SELECT * FROM incidents WHERE status = ? ORDER BY created_at DESC').all(status.toUpperCase());
    }
...
async function CloseIncident(call, callback) {
  try {
