...
async function CreateRule(call, callback) {
  try {
    ...
    await db.prepare(`INSERT INTO rules (id, name, severity_threshold, service_name, channel, created_at) VALUES (?, ?, ?, ?, ?, ?)`)
...
async function ListRules(call, callback) {
  try {
    const rules = await db.prepare('SELECT * FROM rules ORDER BY created_at DESC').all();
    callback(null, { rules });
  } catch (err) {
    callback(null, { rules: [] });
  }
...
async function DeleteRule(call, callback) {
  try {
...
async function EvaluateIncident(call, callback) {
  try {
    ...
    for (const rule of rules) {
      ...
      if (incidentLevel >= ruleLevel && serviceMatch) {
        ...
        const now = new Date().toISOString();
        await db.prepare(`INSERT OR IGNORE INTO alerts (id, incident_id, rule_id, rule_name, channel, triggered_at) VALUES (?, ?, ?, ?, ?, ?)`)
